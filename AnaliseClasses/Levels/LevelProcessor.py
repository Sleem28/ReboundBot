...
class LevelProcessor:
    ...
    async def find_levels(self, symbol: str, tf: str, limit: int) -> list:
        ...
        while start_index > 2:
            ...
            cur_candle = await self.__create_candle(candles.iloc[start_index - 3])
    ...
    def __find_level_pattern(self, first_candle: Candle,
        ...
        if first_candle.volume > vol_lvl or second_candle.volume > vol_lvl or third_candle.volume > vol_lvl:
            height_second_candle = second_candle.high - second_candle.low  # Высота свечи
            up_third_part_of_sc = second_candle.high - height_second_candle / 3  # Цена верхней трети
            dn_third_part_of_sc = height_second_candle / 3 + second_candle.low  # Цена нижней трети

            if third_candle.open < third_candle.close and first_candle.close <= dn_third_part_of_sc:  # Верхний паттерн
                if second_candle.high > first_candle.high and second_candle.high > third_candle.high:
                    return LevelType.TYPE_UP
            elif third_candle.open > third_candle.close and first_candle.open >= up_third_part_of_sc:  # Нижний паттерн
        ...
        for lvl in self.__active_levels:
            if not lvl.level_validate:
                if lvl.level_type == LevelType.TYPE_DOWN:
                    dn_valid_price = last_candle.high - (last_candle.high * 0.01 * self.__VALIDATE_PERCENT)
                    if lvl.level_price < dn_valid_price:
                        lvl.level_validate = True
                elif lvl.level_type == LevelType.TYPE_UP:
                    up_valid_price = last_candle.low + (last_candle.low * 0.01 * self.__VALIDATE_PERCENT)
                    if lvl.level_price > up_valid_price:
                        lvl.level_validate = True
